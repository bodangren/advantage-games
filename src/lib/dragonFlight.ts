...
const createGateRound = (vocabulary: VocabularyItem[], rng: () => number): DragonFlightRound => {
...
export const createDragonFlightState = (
  ...
  { durationMs = DEFAULT_DURATION_MS, rng = Math.random }: DragonFlightConfig = {}
): DragonFlightState => ({
  status: 'running',
  durationMs,
  elapsedMs: 0,
...
export const getDragonFlightResults = ({
  ...
  totalAttempts,
  dragonCount,
}: DragonFlightResultInput): DragonFlightResults => {
