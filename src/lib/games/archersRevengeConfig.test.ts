...
describe("archersRevengeConfig", () => {
  ...
  describe("getDifficultySettings", () => {
    ...
    it("should return extreme settings", () => {
      const settings = getDifficultySettings("extreme");
      expect(settings.playerHp).toBe(1);
      expect(settings.rows).toBe(5);
      expect(settings.enemySpeed).toBe(65);
      expect(settings.descendSpeed).toBe(35);
    });
  });
});
