...
export function WizardZombieGame({ vocabulary, onComplete }: WizardZombieGameProps) {
  ...
  useInterval(() => {
    if (gameState && gameState.status === 'playing') {
        setGameState(prev => prev ? advanceWizardZombieTime(prev, 50, input) : null)
    }
            ...
            <>
                ...
                <div className="absolute bottom-8 right-8 z-20">
                    <VirtualDPad onInput={setVirtualInput} />
                                                ...
                                                <Stage width={dimensions.width} height={dimensions.height} data-testid="stage"><Layer scaleX={dimensions.scale} scaleY={dimensions.scale} x={(dimensions.width - GAME_WIDTH * dimensions.scale) / 2} y={(dimensions.height - GAME_HEIGHT * dimensions.scale) / 2}><Rect width={GAME_WIDTH} height={GAME_HEIGHT} fill="#334155" /><Group><Circle x={gameState.player.x} y={gameState.player.y} radius={gameState.player.radius} fill="#3b82f6" name="player" />{gameState.orbs.map(orb => (<Group key={orb.id} x={orb.x} y={orb.y}><Circle radius={orb.radius} fill={orb.isCorrect ? "#eab308" : "#94a3b8"} name="orb" /><Text text={orb.translation} fontSize={14} fill="white" offsetX={orb.radius} offsetY={7} width={orb.radius * 2} align="center" /></Group>))}</Group></Layer></Stage>
                </>
            )}
        </div>
      )
