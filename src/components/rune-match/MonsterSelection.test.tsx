...
describe('MonsterSelection', () => {
  ...
  it('displays monster stats correctly', () => {
    ...
    expect(screen.getByText((content) => content.includes(`${goblinConfig.hp} HP`))).toBeInTheDocument()
    expect(screen.getByText((content) => content.includes(`1-${goblinConfig.attack}`))).toBeInTheDocument()
    expect(screen.getByText((content) => content.includes(`${goblinConfig.xp} XP`))).toBeInTheDocument()
  })
