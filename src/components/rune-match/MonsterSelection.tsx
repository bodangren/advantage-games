...
export function MonsterSelection({ onSelect }: MonsterSelectionProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 w-full max-w-5xl mx-auto space-y-8 animate-in fade-in zoom-in duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
        {monsterTypes.map((type) => {
          ...
          return (
            <Card 
              key={type}
              className={`bg-slate-900/50 border-2 transition-all hover:scale-105 ${meta.color} hover:border-current cursor-default group overflow-hidden`}
            >
              <div className="h-32 w-full overflow-hidden bg-slate-950/50 flex items-center justify-center relative">
                <div 
                  className="w-24 h-24 transition-transform group-hover:scale-110 duration-500"
                  style={{
