      ...
      <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">{gameTitle}</h2>
            </div>
          </div>
          {gameSubtitle ? (
              ...
              {hasInstructions ? (
                <ul className="space-y-3 text-sm text-slate-300">
                  {instructions?.map((instruction, index) => {
                    ...
                    const stepLabel = String(instruction.step ?? index + 1).padStart(2, '0')
                    return (
                      <li key={`${instruction.text}-${index}`} className="flex gap-3">
                        <span className="text-amber-400 font-bold">{stepLabel}.</span>
                ...
                <p className="text-sm text-slate-400">Instructions will appear once the game is ready.</p>
              )}
            </div>
        ...
        {hasControls ? (
          <div className="hidden sm:flex items-center gap-6 text-[10px] uppercase tracking-[0.2em] text-white/50 sm:text-xs">
            {controls?.map((control, index) => (
              <div key={`${control.label}-${index}`} className="flex items-center gap-2">
