...
describe('DragonRiderGame', () => {
  it('renders the running state with prompt and dragon count', () => {
    ...
    render(<DragonRiderGame vocabulary={vocabulary} preloadedAssets={assets} />)
    ...
    expect(screen.getByTestId('dragon-rider')).toHaveAttribute('data-status', 'running')
    expect(screen.getByTestId('dragon-rider-dragon-count')).toHaveTextContent('1')
  ...
  it('updates dragon count after selecting a correct gate', () => {
    ...
    expect(screen.getByTestId('dragon-rider-dragon-count')).toHaveTextContent('2')
