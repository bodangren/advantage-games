...
export default function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  }, [isAuthenticated, params, getClass, router, user?.id]);

  const handleArchive = async () => {
    if (classId && window.confirm('Are you sure you want to archive this class?')) {
              ...
              <div className="pt-4 border-t">
                ...
                <div className="bg-white p-2 rounded inline-block">
                  {typeof window !== 'undefined' && (
                    <QRCodeSVG
                      value={`${window.location.origin}/join?code=${classData.enrollmentCode}`}
                      size={128}
                      level="M"
                      includeMargin={false}
                    />
