...
Audit the **Sorcerer Ziggurat** game against the 25 shared specifications that have been standardized across all vocabulary/sentence games in the advantage-games platform (March–April 2026). This track produces a pass/fail compliance report and fixes any non-compliant items.

## Game Under Audit

| Property | Value |
|----------|-------|
| **Game ID** | `sorcerer-ziggurat` |
| **Title** | Sorcerer Ziggurat |
| **Type** | sentence |
| **Component** | `src/components/games/sentence/sorcerer-ziggurat/SorcererZigguratGame.tsx` |
| **Logic** | `src/lib/games/sorcererZiggurat.ts` |
| **Page** | `src/app/[locale]/(student)/student/games/sentence/sorcerer-ziggurat/page.tsx` |
| **API** | `src/app/api/v1/games/sorcerer-ziggurat/sentence/route.ts` |
| **Assets** | `public/games/sentence/sorcerer-ziggurat/` |
...
- [ ] **Sentence Data** — Uses `SentenceItem[]` with `{ term, translation }`.
- [ ] **API Route Factories** — Uses `createSentenceRoute` / `createCompleteRoute` from `@/lib/games/api`.
...
- [ ] **Asset Location** — PNGs/sprite sheets in `/public/games/sentence/sorcerer-ziggurat/`.
- [ ] **Cover Image** — Cover image at `/public/games/cover/sorcerer-ziggurat-cover.png`.
...
- [ ] Final compliance report written to `measure/tracks/sorcerer-ziggurat-compliance-audit_20260426/report.md`.
