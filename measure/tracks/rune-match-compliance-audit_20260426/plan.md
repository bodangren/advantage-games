...
- [ ] Task: Read game source files (RuneMatchGame.tsx, runeMatch.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/runeMatch.ts' --collectCoverageFrom='src/components/games/vocabulary/rune-match/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/vocabulary/rune-match/RuneMatchGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in RuneMatchGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in runeMatch.ts.
...
- [ ] Task: Commit all changes with `chore(audit): Rune Match compliance audit complete`.
