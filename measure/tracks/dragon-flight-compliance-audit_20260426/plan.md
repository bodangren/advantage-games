...
- [x] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/dragonFlight.ts' --collectCoverageFrom='src/components/games/vocabulary/dragon-flight/**/*.tsx'`).
- [x] Task: Record lint status (`npm run lint -- --file src/components/games/vocabulary/dragon-flight/DragonFlightGame.tsx`).
