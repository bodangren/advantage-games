...
- [ ] Task: Read game source files (DragonFlightGame.tsx, dragonFlight.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/dragonFlight.ts' --collectCoverageFrom='src/components/games/vocabulary/dragon-flight/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/vocabulary/dragon-flight/DragonFlightGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in DragonFlightGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in dragonFlight.ts.
...
- [ ] Task: Commit all changes with `chore(audit): Dragon Flight compliance audit complete`.
