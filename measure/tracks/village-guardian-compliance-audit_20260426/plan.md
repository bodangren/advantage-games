...
- [ ] Task: Read game source files (VillageGuardianGame.tsx, villageGuardian.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/villageGuardian.ts' --collectCoverageFrom='src/components/games/sentence/village-guardian/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/sentence/village-guardian/VillageGuardianGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in VillageGuardianGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in villageGuardian.ts.
...
- [ ] Task: Commit all changes with `chore(audit): Village Guardian compliance audit complete`.
