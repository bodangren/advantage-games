...
- [ ] Task: Read game source files (AstralMageGame.tsx, astralMage.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/astralMage.ts' --collectCoverageFrom='src/components/games/sentence/astral-mage/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/sentence/astral-mage/AstralMageGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in AstralMageGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in astralMage.ts.
...
- [ ] Task: Commit all changes with `chore(audit): Astral Mage compliance audit complete`.
