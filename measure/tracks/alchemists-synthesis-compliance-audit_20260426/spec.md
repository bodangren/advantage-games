...
Audit the **Alchemists Synthesis** game against the 25 shared specifications that have been standardized across all vocabulary/sentence games in the advantage-games platform (March–April 2026). This track produces a pass/fail compliance report and fixes any non-compliant items.

## Game Under Audit

| Property | Value |
|----------|-------|
| **Game ID** | `alchemists-synthesis` |
| **Title** | Alchemists Synthesis |
| **Type** | vocabulary |
| **Component** | `src/components/games/vocabulary/alchemists-synthesis/AlchemistsSynthesisGame.tsx` |
| **Logic** | `src/lib/games/alchemistsSynthesis.ts` |
| **Page** | `src/app/[locale]/(student)/student/games/vocabulary/alchemists-synthesis/page.tsx` |
| **API** | `src/app/api/v1/games/alchemists-synthesis/vocabulary/route.ts` |
| **Assets** | `public/games/vocabulary/alchemists-synthesis/` |
...
- [ ] **Vocabulary Data** — Uses `VocabularyItem[]` with `{ term, translation }`.
- [ ] **API Route Factories** — Uses `createVocabularyRoute` / `createCompleteRoute` from `@/lib/games/api`.
...
### Project Integration
- [ ] **Game Registry** — Registered in `src/lib/gameCards.ts` with correct `type: 'vocabulary'` and `status: 'playable'`.
- [ ] **Asset Location** — PNGs/sprite sheets in `/public/games/vocabulary/alchemists-synthesis/`.
- [ ] **Cover Image** — Cover image at `/public/games/cover/cover-alchemists-synthesis.png`.
...
- [ ] Final compliance report written to `measure/tracks/alchemists-synthesis-compliance-audit_20260426/report.md`.
