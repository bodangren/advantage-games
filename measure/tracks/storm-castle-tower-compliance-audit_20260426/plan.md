# Implementation Plan: Storm the Castle Tower Compliance Audit

## Phase 1: Discovery & Baseline
- [x] Task: Read game source files (StormCastleTowerGame.tsx, stormCastleTower.ts, page.tsx, API routes). [70d4f01]
- [x] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/stormCastleTower.ts' --collectCoverageFrom='src/components/games/sentence/storm-castle-tower/**/*.tsx'`). [70d4f01]
- [x] Task: Record lint status (`npm run lint -- --file src/components/games/sentence/storm-castle-tower/StormCastleTowerGame.tsx`). [70d4f01]
...
- [x] Task: Verify React-Konva usage in StormCastleTowerGame.tsx. [70d4f01]
- [x] Task: Verify mobile-first portrait (390×844) responsive scaling. [70d4f01]
- [x] Task: Verify pure state + tick function pattern in stormCastleTower.ts. [70d4f01]
...
- [x] Task: Commit all changes with `chore(audit): Storm the Castle Tower compliance audit complete`. [70d4f01]
