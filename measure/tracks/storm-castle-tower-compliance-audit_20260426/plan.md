...
- [ ] Task: Read game source files (StormCastleTowerGame.tsx, stormCastleTower.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/stormCastleTower.ts' --collectCoverageFrom='src/components/games/sentence/storm-castle-tower/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/sentence/storm-castle-tower/StormCastleTowerGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in StormCastleTowerGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in stormCastleTower.ts.
...
- [ ] Task: Commit all changes with `chore(audit): Storm the Castle Tower compliance audit complete`.
