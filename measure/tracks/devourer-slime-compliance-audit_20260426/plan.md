...
- [ ] Task: Read game source files (DevourerSlimeGame.tsx, devourerSlime.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/devourerSlime.ts' --collectCoverageFrom='src/components/games/sentence/devourer-slime/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/sentence/devourer-slime/DevourerSlimeGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in DevourerSlimeGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in devourerSlime.ts.
