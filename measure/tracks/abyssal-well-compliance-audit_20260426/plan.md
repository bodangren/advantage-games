...
- [ ] Task: Read game source files (AbyssalWellGame.tsx, abyssalWell.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/abyssalWell.ts' --collectCoverageFrom='src/components/games/sentence/abyssal-well/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/sentence/abyssal-well/AbyssalWellGame.tsx`).
...
- [ ] Task: Verify React-Konva usage in AbyssalWellGame.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in abyssalWell.ts.
...
- [ ] Task: Commit all changes with `chore(audit): The Abyssal Well compliance audit complete`.
