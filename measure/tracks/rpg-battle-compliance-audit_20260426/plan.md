...
- [ ] Task: Read game source files (BattleScene.tsx, rpgBattleWordSelection.ts, page.tsx, API routes).
- [ ] Task: Run existing tests and record current coverage (`CI=true npm test -- --coverage --collectCoverageFrom='src/lib/games/rpgBattleWordSelection.ts' --collectCoverageFrom='src/components/games/vocabulary/rpg-battle/**/*.tsx'`).
- [ ] Task: Record lint status (`npm run lint -- --file src/components/games/vocabulary/rpg-battle/BattleScene.tsx`).
...
- [ ] Task: Verify React-Konva usage in BattleScene.tsx.
- [ ] Task: Verify mobile-first portrait (390×844) responsive scaling.
- [ ] Task: Verify pure state + tick function pattern in rpgBattleWordSelection.ts.
...
- [ ] Task: Commit all changes with `chore(audit): RPG Battle compliance audit complete`.
